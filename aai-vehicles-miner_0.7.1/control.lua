...
local util = require("__aai-vehicles-miner__.util")
...
function Prototypes.item_prototype(name)
  if not Prototypes.item_prototypes[name] then
    Prototypes.item_prototypes[name] = prototypes.item[name]
  end
  return Prototypes.item_prototypes[name]
end

Prototypes.recipe_prototypes = {}
function Prototypes.recipe_prototype(name)
  if not Prototypes.recipe_prototypes[name] then
    Prototypes.recipe_prototypes[name] = prototypes.recipe[name]
  end
  return Prototypes.recipe_prototypes[name]
end

Prototypes.entity_prototypes = {}
function Prototypes.entity_prototype(name)
  if not Prototypes.entity_prototypes[name] then
    Prototypes.entity_prototypes[name] = prototypes.entity[name]
  end
  return Prototypes.entity_prototypes[name]
end
...
local get_fuel = {
    ...
    build = function()
        return util.table_map(prototypes.item,
            function(name, item)
                local fuel_item = {}
    ...
    item = function(contents)
      ...
      for _, item_stack in pairs(contents) do
          if fuel_items[item_stack.name] then
              return fuel_items[item_stack.name]
            end
        end
    end
}
...
local unbarrel_fluid_recipes = prototypes.get_recipe_filtered{
  -- Build a local list of recipes with product barrel or canister
  {
    filter = "has-product-item", elem_filters = { {filter = "name", name = "barrel"}, {filter="name", name="canister"}}
}
}


local get_fluid_recipe = {
  -- Return a recipe if a recipe producing the fluid "fluid_name" is found in local variable
  -- AND the vehicle storage contains its ingredients.
  from_contents = function(fluid_name, inventory)
      if not unbarrel_fluid_recipes then
          return nil -- Safely handle missing or nil `unbarrel_fluid_recipes`.
      end
      for _, recipe in pairs(unbarrel_fluid_recipes) do
          -- Ensure recipe and its products are valid
          if recipe.products and recipe.products[2] and recipe.products[2].name == fluid_name then
              local ingredient = recipe.ingredients[1]
              -- Check inventory for the first ingredient (we assume there is only one ingredient in those recipes)
              if ingredient and inventory.get_item_count(ingredient.name) > 0 then
                  return recipe
              end
          end
      end
...
function miners:new(entity)
    if entity and entity.valid and entity.type == "car" then
      ...
      if self.variants[effective_name] then
        ...
        --metatables are not saved to storage (formerly global). Iterate and re-assign in on_load
...
local function barrel_fluids(miner)
  if miner.fluids then
    ...
    for _, fluid in pairs(miner.fluids) do
      local recipe = Prototypes.recipe_prototype("fill-" .. fluid.name .. "-barrel") or Prototypes.recipe_prototype("fill-" .. fluid.name .. "-canister")
          ...
          if item_count >= recipe.ingredients[1].amount then
            ...
            if max_cycles > 0 then
              ...
              if fluid.amount > recipe.ingredients[2].amount then
                miner.stopped_on_fluid[fluid.name] = true
              end
            end
          else
            miner.stopped_on_fluid[fluid.name] = true
...
local function mine_resource(miner, resource, required_fluid)
    ...
    local mining_speed = base_mining_speed * variant.mining_speed * storage["vehicle-mining-multiplier"] / 100
    ...
    miner.mining_progress = (miner.mining_progress or 0) + mining_rate_per_sec / 60 * storage["vehicle-mining-interval"]

    -- consume energy
    miner.entity.burner.remaining_burning_fuel = miner.entity.burner.remaining_burning_fuel - variant.mining_energy_use / 60 * storage["vehicle-mining-interval"]

    -- add pollution
    miner.entity.surface.pollute(miner.entity.position, variant.mining_pollution * storage["vehicle-mining-interval"])
...
local function mine_area(miner)
  -- 20
    if (game.tick + miner.entity.unit_number) % storage["vehicle-mining-interval"] == 0 then
        ...
        for _, resource in pairs(try_resources) do
          ...
          if not storage.disallowed_resource_categories[proto.resource_category] then
        ...
        --local exclude_types = {}
        local exclude_types = table.deepcopy(storage.disallow_resources)
...
local function consume_fuel_or_equipment(miner)
    if miner.entity.grid and miner.entity.grid.available_in_batteries > 10000 then
        ...
        miner.entity.burner.currently_burning = high_fuel_item
        local energy_deficit = Prototypes.item_prototype(high_fuel_item).fuel_value - miner.entity.burner.remaining_burning_fuel --use initial energy value as baseline
        local batteries = {}
        for _, equipment in pairs(miner.entity.grid.equipment) do
          if equipment.type == "battery-equipment" then
            table.insert(batteries, equipment)
          end
        end
...
local function collect_floor_items(miner)
    if (game.tick + miner.entity.unit_number) % storage["vehicle-collection-interval"] == 0 then
        ...
        for _, ent in pairs(util.table_filter(miner.entity.surface.find_entities(area), _filter)) do
            if ent.type == "item-entity" then
                ...
                if ent.prototype.mineable_properties and ent.prototype.mineable_properties.products then
...
local function on_tick()
    ...
    for _, miner in pairs(miners.miners) do
        if miner.entity.valid and ( (not storage["vehicle-mining-requires-movement"]) or (miner.entity.speed > 0.001 or miner.entity.speed < -0.001)) then

            if (not miner.entity.burner.currently_burning) or (miner.entity.burner.remaining_burning_fuel < miner.entity.burner.currently_burning.name.fuel_value * 0.02 + 1) then
...
local function update_disallowed_resources()
  ...
  if settings.global["vehicle-mining-disallow-resources"] and settings.global["vehicle-mining-disallow-resources"].value then
    ...
    for _, resource in pairs(resource_words) do
      if Prototypes.entity_prototype(resource) then
        storage.disallow_resources[resource] = true
...
local function update_disallowed_resource_categories()
  storage.disallowed_resource_categories = {}
  for interface, functions in pairs(remote.interfaces) do
    if functions["aai_vehicles_miner_disallowed_resource_categories"] then
      local added_categories = remote.call(interface, "aai_vehicles_miner_disallowed_resource_categories")
      for _, resource_category in pairs(added_categories) do
        storage.disallowed_resource_categories[resource_category] = interface
      end
    end
  end
end
...
local function on_runtime_mod_setting_changed()
  ...
  storage["vehicle-mining-multiplier"] = settings.global["vehicle-mining-multiplier"].value
  storage["vehicle-mining-interval"] = settings.global["vehicle-mining-interval"].value
  storage["vehicle-mining-requires-movement"] = settings.global["vehicle-mining-requires-movement"].value
  storage["vehicle-collection-interval"] = settings.global["vehicle-collection-interval"].value
...
script.on_event(defines.events.on_built_entity, function(event) miners:new(event.entity) end)
...
local function on_configuration_changed()
    ...
    for _, miner in pairs(miners.miners) do
        ...
        miner.stopped_on_fluid = miner.stopped_on_fluid or {}
    end

    --update fuel items every change
    storage.fuel_items = get_fuel.build()

    update_disallowed_resource_categories()
    update_disallowed_resources()
...
local function on_init()
    storage.miners = storage.miners or {}
    miners.miners = storage.miners
    storage.fuel_items = get_fuel.build()
    ...
    if settings.startup["start-with-vehicle-miner"].value == true and remote.interfaces["freeplay"] and remote.interfaces["freeplay"]["set_ship_items"] then
	    remote.call("freeplay", "set_ship_items", {["vehicle-miner"]=1})
    end
...
local function write_storage()
    helpers.write_file("AAI/miner.storage.lua", serpent.dump(storage, {comment=false, sparse=true, indent = "\t", nocode=true, name="storage"}), false)
    helpers.write_file("AAI/miner.miners.lua", serpent.dump(miners, {comment=false, sparse=true, indent = "\t", nocode=true, name="miners"}), false)
    helpers.write_file("AAI/miner._ENV.lua", serpent.block(_ENV, {comment=false, sparse=true, indent = "\t", nocode=true}), false)
end
...
remote.add_interface(
    "aai-vehicles-miner",
    {
        ...
        -- example to disable a resource category
        --aai_vehicles_miner_disallowed_resource_categories = function(event) return {"basic-fluid"} end,
