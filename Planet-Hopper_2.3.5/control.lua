...
script.on_event(defines.events.on_rocket_launch_ordered, function(event)
	...
	storage.silos[silo.unit_number] = storage.silos[silo.unit_number] or {}
	...
	if destination_override then
		if not game.surfaces[destination_override] then
			game.planets[destination_override].create_surface()
		end
...
script.on_event(defines.events.on_gui_opened, function(event)
	if not settings.global["allow-destination-override"].value or script.active_mods["Space-Age-Without-Platforms"] then
		return
	end
